...
const OnlineJudge = ({ problem, theme, onBack }) => {
  ...
  const [mobileTab, setMobileTab] = useState("statement"); // statement, editor, output
  ...
  const renderStatement = () => (
    <div className="dashboard-card mobile-scrollable-card" style={{ padding: '1.5rem', flex: 1, overflowY: 'auto', position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', borderBottom: '1px solid var(--glass-border)', paddingBottom: '1rem' }}>
        <h2 style={{ fontFamily: 'var(--font-display)', color: 'var(--dash-text-color)', margin: 0, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          {isMobile && (
            <button onClick={onBack} style={{ background: 'none', border: 'none', color: 'var(--dash-text-color)', cursor: 'pointer', padding: '0 0.5rem 0 0' }}>
      ...
      <div style={{ color: 'var(--dash-text-color)', lineHeight: '1.6', fontSize: '1.1rem', paddingBottom: isMobile ? '80px' : '0' }} dangerouslySetInnerHTML={{ __html: problem.question_text }} />
      
      {problem.testCases && problem.testCases.length > 0 && (
        <div style={{ marginTop: '1.5rem', paddingBottom: isMobile ? '80px' : '0' }}>
      ...
      {isMobile && (
        <div className="mobile-floating-action-bar">
          <button className="floating-play-pill" onClick={() => setMobileTab("editor")}>
  ...
  const renderConsole = () => (
    <div className="dashboard-card" style={{ display: 'flex', flexDirection: 'column', height: isMobile ? '100%' : '250px', flexShrink: 0, overflow: 'hidden', background: theme === 'dark' ? '#1e1e1e' : '#f5f5f5' }}>
      <div style={{ padding: '0.8rem 1.5rem', borderBottom: `1px solid ${theme === 'dark' ? '#333' : '#ddd'}`, display: 'flex', alignItems: 'center', gap: '0.5rem', color: theme === 'dark' ? '#ccc' : '#333', background: theme === 'dark' ? '#252526' : '#eaeaea' }}>
        {isMobile && (
          <button onClick={() => setMobileTab("editor")} style={{ background: 'none', border: 'none', color: 'currentColor', cursor: 'pointer', padding: '0 0.5rem 0 0', display: 'flex', alignItems: 'center' }}>
  ...
  const renderEditor = () => (
    <div className="dashboard-card" style={{ display: 'flex', flexDirection: 'column', flex: isMobile ? 1 : 1.2, height: isMobile ? '100%' : 'auto', minHeight: isMobile ? 'auto' : '400px', overflow: 'hidden' }}>
      <div style={{ padding: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem', borderBottom: '1px solid var(--glass-border)', background: 'var(--glass-bg)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          {isMobile && (
            <button onClick={() => setMobileTab("statement")} style={{ background: 'none', border: 'none', color: 'var(--dash-text-color)', cursor: 'pointer', padding: '0 0.5rem 0 0', display: 'flex', alignItems: 'center' }}>
              <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none"><polyline points="15 18 9 12 15 6"></polyline></svg>
            </button>
          )}
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button 
            ...
            style={{ padding: '0.4rem 0.8rem', minHeight: 'auto', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 'bold', background: 'transparent', border: '1px solid var(--primary-accent)', color: 'var(--primary-accent)' }}
          ...
          <button 
            className="subject-selection-btn" 
            style={{ padding: '0.4rem 0.8rem', minHeight: 'auto', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 'bold' }}
            onClick={runCode}
            disabled={loading}
          >
            {loading ? "..." : "Run"}
  ...
  if (isMobile) {
    return (
      <div className="cs24102-mobile-container" style={{ height: 'calc(100vh - 100px)', padding: '1rem' }}>
        {mobileTab === "statement" && renderStatement()}
        {mobileTab === "editor" && renderEditor()}
        {mobileTab === "output" && renderConsole()}
      </div>
    );
  }
